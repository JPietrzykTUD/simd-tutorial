...
template<typename T>
auto group_sum_neon(simple_map_soa<T> & dst, T const * __restrict__ group_keys, T const * __restrict__ group_values, size_t element_count) -> size_t;
  ...
  while (group_keys != end) {
    ...
    while (true) {
      ...
      if (key_found_min_mask_value <= 4) {
        dst_values[*(map_values + pos_hint + key_found_min_mask_value - 1)] += value;
        break;
      }
...
auto group_sum_neon(
  ...
  auto const end = group_keys + element_count;
  auto const empty_bucket_vec = vdupq_n_u64(dst.empty_bucket_value);
  auto const all_false_mask = 0;
  // create a vector with values ~1, ~2
  std::array<uint64_t, 2> mask_values{{ 0xfffffffe, 0xfffffffd }};
  auto const mask_value_vec = vld1q_u64(mask_values.data());

  while (group_keys != end) {
    auto const key = *group_keys;
    auto const key_vec = vdupq_n_u64(key);
    auto const value = *group_values;
    // knuths multiplicative hash method
    auto pos_hint = static_cast<uint64_t>(key)*2654435761ull;
    while (true) {
      pos_hint = pos_hint % map_element_count;
      auto map_keys_vec = vld1q_u64(map_keys + pos_hint);
      auto const key_found_vec = vceqq_u64(map_keys_vec, key_vec);
      auto const key_found_xor_mask_value_vec = veorq_u64(key_found_vec, mask_value_vec);
      auto const key_found_min_mask_value = vminvq_u64(key_found_xor_mask_value_vec);
      if (key_found_min_mask_value <= 2) {
        dst_values[*(map_values + pos_hint + key_found_min_mask_value - 1)] += value;
        break;
      }
      auto const empty_bucket_found_vec = vceqq_u64(map_keys_vec, empty_bucket_vec);
      auto const empty_bucket_xor_mask_value_vec = veorq_u64(empty_bucket_found_vec, mask_value_vec);
      auto const empty_bucket_min_mask_value = vminvq_u64(empty_bucket_xor_mask_value_vec);
      if (empty_bucket_min_mask_value <= 2) {
        auto pos = pos_hint + empty_bucket_min_mask_value - 1;
        map_keys[pos] = key;
        map_values[pos] = groups_count;
        dst_keys[groups_count] = key;
