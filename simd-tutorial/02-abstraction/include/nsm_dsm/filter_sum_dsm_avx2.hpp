...
template<typename T>
void filter_eq_sum_dsm_avx2(T * __restrict__ dst, table_dsm<T> const & data, T const value);

template<>
FORCE_INLINE void filter_eq_sum_dsm_avx2(
  uint32_t * __restrict__ dst, 
  table_dsm<uint32_t> const & data,
...
FORCE_INLINE void filter_eq_sum_dsm_avx2(
  ...
  table_dsm<uint64_t> const & data,
  uint64_t const value
) {
  auto element_count = data.row_count;
  uint64_t const * to_filter = data.col0();
  uint64_t const * to_sum = data.col1();
